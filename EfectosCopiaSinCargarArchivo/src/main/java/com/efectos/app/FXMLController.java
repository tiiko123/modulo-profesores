...
public class FXMLController implements Initializable {
    ...
    @FXML
    ImageView upv, letras, loading, upv2, letras2;
    @FXML
    Label error, labelMos,errorRegistrar,registroExitoso;
    @FXML
    Button siguienteButton,siguienteSeleccionCuenta,registerB,volver,siguienteAdministrador;
    @FXML
    ComboBox<String> comboBox,comboBoxTipo;
    @FXML
    TextField usuarioAl,usuarioAd,usuarioRegistrar;
    @FXML
    PasswordField contraseniaAl,contraseniaAd,contraseniaRegistrar,contraseniaRegistrarConfirmar;
    @FXML
    Button disp_horarios,eva_mat,act_hora,cerrar_sesion;
    String comboBoxDatos="";
    int cont=1;
    int cont2=1;
    String obtenerPrimerNumero="";
    @FXML
    ...
    String direccion = "/home/ahren/Escritorio/EfectosCopiaSinCargarArchivo/src/main/resources/texto/usuarios.txt";
    LecturaUsuarios lectura = new LecturaUsuarios(direccion);

   

    /*/ APARTIR DE AQUI ES EL CODIGO DEL INICIO DE SESION
        "ADMIN" SE REFIESE A LOS DIRECTORES DE CARRERA FALTA CORREGIR, "ALUMNO" SERIA EL PROFESOR
        EL APARTADO DE REGISTRARSE FALTA POR QUITAR
        POR EL MOMENTO LOS USUARIOS SON ADMIN MAS ADELANTE SE MODIFICARAN
        LA LISTA DE USUARIOS ESTA EN UN TXT SE NECESITA CONECTAR A UNA BD
    /*/
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        //SI SE QUIEREN SALTAR LA ANIMACION COMENTEN DESDE ACA...

        FadeTransition fadeOut = new FadeTransition(Duration.seconds(0.0001), upv);
        fadeOut.setFromValue(1.0);
        fadeOut.setToValue(0.0);
        fadeOut.play();

        FadeTransition fadeOut2 = new FadeTransition(Duration.seconds(0.0001), letras);
        fadeOut2.setFromValue(1.0);
        fadeOut2.setToValue(0.0);
        fadeOut2.play();

        FadeTransition fadeOut3 = new FadeTransition(Duration.seconds(0.0001), siguienteButton);
        ...
        TimerTask cargandoOcultar = new TimerTask() {
            @Override
            public void run() {

                FadeTransition fadeOut3a = new FadeTransition(Duration.seconds(1), loading);
                fadeOut3a.setFromValue(1.0);
                fadeOut3a.setToValue(0.0);
                fadeOut3a.play();
            }
        };
        timer.schedule(cargandoOcultar, 3000);

        TimerTask upvMostrar = new TimerTask() {

            @Override
            public void run() {
                FadeTransition fadeIna = new FadeTransition(Duration.seconds(3), upv);
                fadeIna.setFromValue(0.0);
                fadeIna.setToValue(1.0);
                fadeIna.play();
            }
        };
        timer.schedule(upvMostrar, 5000);

        TimerTask letrasMostrar = new TimerTask() {
            @Override
            public void run() {

                FadeTransition fadeIn2a = new FadeTransition(Duration.seconds(3), letras);
                fadeIn2a.setFromValue(0.0);
                fadeIn2a.setToValue(1.0);
                fadeIn2a.play();

            }
        };
        timer.schedule(letrasMostrar, 6000);

        TimerTask botonMostrar = new TimerTask() {
            @Override
            public void run() {

                FadeTransition fadeIn4a = new FadeTransition(Duration.seconds(0.001), siguienteButton);
                fadeIn4a.setFromValue(0.0);
                fadeIn4a.setToValue(1.0);
                fadeIn4a.play();

            }
        };
        timer.schedule(botonMostrar, 11000);
        // HASTA ACA

    }


    public void siguienteInicio(){
        app.fxml2();
        Stage stage = (Stage) this.siguienteButton.getScene().getWindow();
        stage.close();
    }
    ...
    public void combo(){
        if(cont==1){
            comboBox.getItems().addAll("Alumno","Administrador");
    ...
    public void combo2(){
        if(cont2==1){
            comboBoxTipo.getItems().addAll("Mobiliario","E. Computo");

            cont2++;
        }
    }
    
    public void siguienteInicio2(){
        Stage stage = (Stage) this.siguienteSeleccionCuenta.getScene().getWindow();
        comboBoxDatos=String.valueOf(comboBox.getValue());
        System.out.println("Seleccionado: "+comboBoxDatos);

        if(comboBoxDatos.equals("null")){
            error.setText("Seleccione un tipo de cuenta");
        } else{
            if(comboBoxDatos.equals("Alumno")){
                stage.close();
                app.fxml3();
                stage.close();
            }else{
    ...
    public void ingresarAlumno(){
        ...
        listaUsuarios = lectura.getArray();
        for(int i = 0; i< listaUsuarios.size(); i++) {

            obtenerPrimerNumero = listaUsuarios.get(i).substring(0,1);
            if(obtenerPrimerNumero.equals("2")) {
                if (usuarioAl.getText().equals(listaUsuarios.get(i).substring(listaUsuarios.get(i).indexOf("#") + 1, listaUsuarios.get(i).indexOf("$")))) {
                    if (contraseniaAl.getText().equals(listaUsuarios.get(i).substring(listaUsuarios.get(i).indexOf("$") + 1, listaUsuarios.get(i).length()))) {
                        System.out.println("Entro con todas las condiciones con usuario");
                        System.out.println("abrire menu profes");
                        app.menu_profes();
                        Stage stage = (Stage)this.siguienteAdministrador.getScene().getWindow();
                        stage.close();
                    }
    ...
    public void ingresarAdministrador(){
        ...
        for(int i = 0; i< listaUsuarios.size(); i++) {
            ...
            if(obtenerPrimerNumero.equals("1")) {
                if (usuarioAd.getText().equals(listaUsuarios.get(i).substring(listaUsuarios.get(i).indexOf("#") + 1, listaUsuarios.get(i).indexOf("$")))) {
                    if (contraseniaAd.getText().equals(listaUsuarios.get(i).substring(listaUsuarios.get(i).indexOf("$") + 1, listaUsuarios.get(i).length()))) {
                        app.men_director();
                        Stage stage = (Stage)this.siguienteAdministrador.getScene().getWindow();
                        stage.close();
    ...
    public void registrarse(){
        Stage stage = (Stage) this.registerB.getScene().getWindow();
        stage.close();
        app.fxml4();
    ...
    public void registrarCuenta(){
        ...
        if(!usuarioRegistrar.getText().equals("")){
            if (!contraseniaRegistrar.getText().equals("")){
                if(contraseniaRegistrarConfirmar.getText().equals(contraseniaRegistrar.getText())){

                    try{

                        BufferedWriter bf = new BufferedWriter(new FileWriter("/home/ahren/Escritorio/EfectosCopiaSinCargarArchivo/src/main/resources/texto/usuarios.txt",true));
                        bf.write("\n"+"2#"+usuarioRegistrar.getText()+"$"+contraseniaRegistrar.getText());
                        bf.close();
                        errorRegistrar.setText("");
                        errorRegistrar.setVisible(false);
                        registroExitoso.setText("Registro Exitoso");

                    }catch (Exception e){
                        System.out.println("No agrego we");
                    }
    ...
    public void mensajeError(){
            errorRegistrar.setText("Error al registrar");
            usuarioRegistrar.setText("");
            contraseniaRegistrar.setText("");
            contraseniaRegistrarConfirmar.setText("");
    }

    public void volverDeRegistrar(){
    ...
    public void disponibilidad(){
        System.out.println ("estoy en disponibilidad");
        app.dispo();
        Stage stage = (Stage)this.disp_horarios.getScene().getWindow();
       // stage.close();
    }
    public void evaluar(){
        System.out.println ("estoy en evaluar");
        app.evaluar();
        Stage stage = (Stage)this.eva_mat.getScene().getWindow();
       // stage.close();
    }
...
public void initialize(URL url, ResourceBundle resourceBundle) {
 ...
 public void aProfes(){
    ...
    public void aPrioridad(){
        app.fxmlprioridad();
    }

    public void aDisponibilidad(){
        app.fxmldisponibilidad();
    }
